public class Main {
    ...
    public static String[] positions = {"President", "Vice President", "Secretary", "Treasurer"};
    public static String[][] candidates = {{"abstain", "Mordy Golding", "Shira Gotshalk", "Stephanie Evans", "Ray Villalobos"},
            {"abstain", "Dona Sarkar", "Tyler Ahn", "Joe Camp", "Brandon LeBlanc"},
            {"abstain", "Jason Howard", "Nikki Rubino", "Peggy Fisher", "Neil Edde"},
            {"abstain", "David Gassner", "Sharon Bennett", "Walt Ritscher", "Heather Hurley"}};
	...
	// write your code here
        ...
        String[][] eachVote = new String [4][votes.length];
        System.out.print("Instances of voter fraud: ");
        for (int j=0;j<(votes.length-1);j++) {
            id[j] = votes[j].substring(0,5);
            eachVote[1][j] = votes[j].substring(7,8);
            eachVote[2][j] = votes[j].substring(9,10);
            eachVote[3][j] = votes[j].substring(11,12);
            eachVote[4][j] = votes[j].substring(13,14);
        ...
        for (int i=0;i<(candidates.length-1); i++) {
            System.out.println("\nYour new " + positions[i] + " : " + nameCandidate(positions[i],voteCounter(eachVote,i)));
            for (int i = 0; i<(voting.length-1); i++) {
                System.out.println(candidates[1][i] + " " + voting[i]);
            }
        }
    ...
    public static int voteCounter(String[][] eachVote,int i) {
        for (int j=0;j<(eachVote[i].length-1);j++) {
            switch (eachVote[i][j]) {
                case "0":
                    voting[i][0]++;
                    break;
        ...
        if ((voting[i][0]>voting[i][1]) && (voting[i][0]>voting[i][2]) && (voting[i][0]>voting[i][3]) && (voting[i][0]>voting[i][4])) {
            return 0;
        }
        if ((voting[i][1]>voting[i][0]) && (voting[i][1]>voting[i][2]) && (voting[i][1]>voting[i][3]) && (voting[i][1]>voting[i][4])) {
            return 1;
        }
        if ((voting[i][2]>voting[i][0]) && (voting[i][2]>voting[i][1]) && (voting[i][2]>voting[i][3]) && (voting[i][2]>voting[i][4])) {
            return 2;
        }
        if ((voting[i][3]>voting[i][0]) && (voting[i][3]>voting[i][1]) && (voting[i][3]>voting[i][2]) && (voting[i][3]>voting[i][4])) {
            return 3;
        }
        else  {
            return 4;
